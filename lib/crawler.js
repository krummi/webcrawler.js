...
Crawler.prototype.crawlEntry = function (entry) {
  ...
  this.get(url, function (url, status, body) {
    ...
    if (body === null) {
      ...
      console.log('      status code: '.red + status.toString().yellow);
    } else {
      ...
      try {
        ...
        console.log('    < crawling url: '.green + url.yellow +
          ' - successful, links found: '.green + links.length.toString().yellow);

        // Check to see if we are available to crawl more domains.
        // TODO: This can be optmized a whole lot!
        if (links.length > 0) {
          this.crawlBestDomains();
        }
      } catch (ex) {
        console.log('exception occured while crawling: '.red + url);
      }
    }
...
Crawler.prototype.get = function (link, cb) {
  // TODO: Look at the Content-Type header and discard none text/html!
  request(link, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      cb(link, 200, body);
    } else {
      cb(link, response.statusCode, null);
    }
  });
};
