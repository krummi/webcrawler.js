...
Crawler.prototype.crawlAll = function(domains) {
  for (var i = 0; i < domains.length; i++) {
    ...
    console.log('crawling: ' + domainName);
    if (domain) {
      ...
      if (!domain.robots) {
        ...
        domain.fetchRobots(this.options['user_agent'], function(parser) {
...
Crawler.prototype.crawl = function(domain) {
  ...
  if (domainFrontier.canFetch(
        this.options['user_agent'], parsedUrl.pathname)) {
  ...
  var body = this.get(url, function(body) {
    ...
    _(links).forEach(function(link) {
      ...
      if (resolved && (S(resolved).startsWith('http:') ||
        ...
        // Ignore the URL if it is in the visited list.
        if (canonicalized in this.visited) {
    ...
    this.crawlAll(Object.keys(newDomains));
