...
Crawler.prototype.printReport = function () {
  ...
  for (var i = 0; i < this.results.length; i++) {
    console.log('Query found in page: '.green + results[i].yellow);
  }
  console.log('No of unique links found: '.green +
...
/*
 * Starts crawling a set of new domains.
 */
Crawler.prototype.startCrawling = function () {

  // Start crawling new domains until either reach the maximum number of
  // domains, or the number of domains that we've seen.
  var howMany = Math.min(this.frontier.getNoOfDomains(), this.maxDomains);
...
Crawler.prototype.crawlEntry = function (entry) {
  ...
  console.log('    > crawling url: '.green + url.yellow + " - depth: ".green +
    depth.toString().yellow + " - score: ".green +
    score.toString().yellow);
  ...
  this.get(url, function (url, body) {
    ...
    if (body === null) {
      console.log('    < crawling url: '.green + url.yellow + " - failed!".red);
      return;
    }

    // Extracts the links and keep track of the new domains that we see.
    ...
    _(links).forEach(function (link) {
      ...
      if (resolved && (S(resolved).startsWith('http:') ||
        ...
        if (!(parsedUrl.host in foundDomains)) {
          foundDomains[parsedUrl.host] = true;
        }
...
Crawler.prototype.shouldStop = function () {
  if (this.pagesCrawled >= this.maxPages) {
    if (!this.printedFinalReport) {
      this.printedFinalReport = true;
      this.printReport();
    }
    return true;
  }
