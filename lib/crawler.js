...
/*
 * Crawls all domains which are not being crawled.
 */
Crawler.prototype.crawlAll = function(domains) {
  for (var i = 0; i < domains.length; i++) {
    var domain = domains[i];
    if (this.frontier[domain]) {
      // assert.equal(this.frontier[domain]['isBeingCrawled'], false);
      // assert.ok(this.frontier[domain]['heap'].size() > 0)
      console.log(keys[i] + ': ' + this.frontier.domains[keys[i]]['isBeingCrawled']);
    }
    
  }
};

Crawler.prototype.crawlAll = function(targets) {
